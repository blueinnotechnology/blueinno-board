...
int BLELocalDevice::begin()
{
...
#if defined(ARDUINO_BLUEINNO_AIOT_RP2040)
  pinMode(SPIWIFI_SS, OUTPUT);
  digitalWrite(IOT_RESETN, OUTPUT);
  digitalWrite(SPIWIFI_SS, LOW);
#endif
...
#if defined(ARDUINO_BLUEINNO_AIOT_RP2040)
  digitalWrite(IOT_RESETN, LOW);
  delay(100);
  digitalWrite(IOT_RESETN, HIGH);
  delay(750);
#endif
  ...
  uint16_t res = HCI.reset();
  if (res != 0) {
    Serial.print("HCI.reset() ");
    Serial.println(res);
    end();
  ...
  if (HCI.readLocalVersion(hciVer, hciRev, lmpVer, manufacturer, lmpSubVer) != 0) {
    Serial.println("HCI.readLocalVersion");
    end();
    return 0;
  }

  if (HCI.setEventMask(0x3FFFFFFFFFFFFFFF) != 0) {
    Serial.println("HCI.setEventMask 0x3FFFFFFFFFFFFFFF");
    end();
    return 0;
  }
  if (HCI.setLeEventMask(0x00000000000003FF) != 0) {
    Serial.println("HCI.setEventMask 0x00000000000003FF");
    end();
    return 0;
  }
  ...
  if (HCI.readLeBufferSize(pktLen, maxPkt) != 0) {
    Serial.println("HCI.readLeBufferSize");
    end();
    return 0;
  }
